...
{
    public class BrandUpdateCommandHandler : CommandHandler, ICommandHandler<BrandUpdateCommand>
    {
        private readonly IBrandWriteRepository _repository;

        public BrandUpdateCommandHandler(
            IBrandWriteRepository repository, IUnitOfWork uow) : base(uow)
        {
            _repository = repository;
        }

        public async Task<IResult> Handle(BrandUpdateCommand command, CancellationToken cancellationToken = default)
        {
