...
{
    public class ProductAttributeOptionUpdateCommandHandler : CommandHandler, ICommandHandler<ProductAttributeOptionUpdateCommand>
    {
        private readonly IProductAttributeOptionWriteRepository _repository;

        public ProductAttributeOptionUpdateCommandHandler(
            IProductAttributeOptionWriteRepository repository, IUnitOfWork uow) : base(uow)
        {
            _repository = repository;
        }

        public async Task<IResult> Handle(ProductAttributeOptionUpdateCommand command, CancellationToken cancellationToken = default)
        {
