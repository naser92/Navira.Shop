...
namespace Navira.Shop.Core.Infrastructure
{
    ...
    public class AppDomainTypeFinder : ITypeFinder
    {
        ...
        {
            _fileProvider = fileProvider ?? GlobalData.DefaultFileProvider;
        }
        ...
        protected virtual void LoadMatchingAssemblies(string directoryPath)
        {
            ...
            foreach (var dllPath in _fileProvider.GetFiles(directoryPath, "*.dll"))
            {
                try
                {
                    if (dllPath.ToLower().Contains("stimulsoft"))
                    {
                        continue;
                    }

                    var an = AssemblyName.GetAssemblyName(dllPath);
                    if (an != null && Matches(an.FullName) && !loadedAssemblyNames.Contains(an.FullName))
                    {
                        App.Load(an);
                    }
