namespace Navira.Shop.Core.Caching
{
    ...
    public partial class CacheKey
    {
        ...
        /// <param name="keyObjects">Objects to create parameters</param>
        /// <returns>Cache key</returns>
        public virtual CacheKey Create(string keyPrefix, Func<object, object> createCacheKeyParameters, params object[] keyObjects)
        {
            var prefix = string.IsNullOrWhiteSpace(keyPrefix) ? "" : $"{keyPrefix}:";
            var cacheKey = new CacheKey($"{prefix}{Key}", Prefixes.Select(x => $"{prefix}{x}").ToArray());
