...
const DeliveryAddress = ({ values, updateId, type, title }) => {
  ...
  const { data, isLoading: load, refetch } = useCustomQuery([user, updateId], () => request({ url: `/${user}/${updateId}` },router), { enabled: false, refetchOnWindowFocus: false, select: (data) => (data.data) });
  // Creating Address
