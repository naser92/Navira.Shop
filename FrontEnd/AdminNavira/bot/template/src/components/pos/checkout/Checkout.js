...
const Checkout = ({ loading, mutate, data, errorCoupon, setAppliedCoupon, appliedCoupon, storeCoupon, setStoreCoupon }) => {
  ...
  const { data: addToCartData, isLoading: addToCartLoader, refetch } = useCustomQuery([AddtoCartAPI], () => request({ url: AddtoCartAPI }, router), { refetchOnWindowFocus: false, cacheTime: 0, select: (res) => res?.data });
  // Getting Users data
  const { data: userData, refetch: userRefetch, isLoading: userLoader } = useCustomQuery([user], () => request({ url: user, params: { role: 'consumer', status: 1, paginate: 15, search: customSearch ? customSearch : "" } }, router), { enabled: false, refetchOnWindowFocus: false, select: (data) => data.data.data });
