...
const AllProducts = ({ setFieldValue, values, dispatch, setCartData, CategoryData, getCategoryId, setStateProduct, setGetCategoryId }) => {
  ...
  // Calling Product API
  const { data, refetch, fetchStatus } = useCustomQuery([product, getCategoryId], () => request({ url: product, params: { category_ids: getCategoryId || productParams.category_ids.join(','), search: productParams?.search, status: 1, paginate: 20, page } }, router), { refetchOnWindowFocus: false, select: (data) => data.data });
            ...
            <POSSkeletonLoader /> : <div className="product-section mt-4">
              <Row xxl={4} md={3} sm={2} xs={1} className={`g-4 row-cols-md-3 row-cols-sm-2 row-cols-1 ${sidebarOpen ? 'row-cols-xxl-5 row-cols-lg-4' : 'row-cols-xxl-4'}`}>
                {data?.data?.length > 0 ? data?.data?.map((item) => (
                  ...
                  : <NoDataFound customImage={NoProductImage.src} title={"NoProductFound"} />}
              </Row>
            </div>
          }
