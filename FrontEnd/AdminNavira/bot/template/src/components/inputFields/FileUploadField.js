...
const FileUploadField = ({
  ...
  const getMimeTypeImage = (result) => {
    ...
    if (result?.original_url?.startsWith("http")) return result.original_url;
    if (result?.original_url) return storageURL + result.original_url;
    return result?.asset_url ?? "/assets/images/placeholder.png";
  };
  const ImageShow = () => {
    return (
      <>
