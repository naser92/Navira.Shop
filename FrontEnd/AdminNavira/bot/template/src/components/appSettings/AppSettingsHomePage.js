...
const AppSettingHomePage = forwardRef(
  ({ activeTab, values, setFieldValue }, ref) => {
    ...
    const {data: productData,isLoading: productLoader,refetch,} = useCustomQuery([product],() =>request({url: product,params: {status: 1,search: customSearch ? customSearch : "",paginate:values["values"]?.["products_ids"]?.length > 15? values["values"]?.["products_ids"]?.length: 15,ids: customSearch? null: values["values"]["products_ids"].join() || null, with_union_products: values["values"]?.["products_ids"]?.length? values["values"]?.["products_ids"]?.length >= 15 ? 0 : 1 : 0,},},router),
      {refetchOnWindowFocus: false,select: (res) => res?.data?.data.map((elem) => { return { id: elem.id, name: elem.name, image: elem?.product_thumbnail?.original_url || "/assets/images/placeholder.png", slug: elem?.slug,};}),});
    const { data: categoryData, isLoading: categoryLoader } = useCustomQuery([Category],() => request({ url: Category, params: { status: 1, type: "product" } },router),{ refetchOnWindowFocus: false, select: (res) => res?.data?.data.map((elem) => { return { id: elem.id, name: elem.name, image: elem?.category_icon?.original_url || "/assets/images/placeholder.png", slug: elem?.slug,};}),});
