    ...
    <>
      {data?.original_url ? (
        <div className={`${!noPrevClass ? "user-profile" : ""} ${customClass ? customClass : ""}`}>
          <Image src={data?.original_url} className={`${customClass ? customClass : ""} ${imageClass ? imageClass : ""}`} height={height || 50} width={width || 50} alt={name?.name || ""} unoptimized/>
        </div>
      ) : placeHolder ? (
        <div className={`user-profile user-round ${customClass ? customClass : ""}`}>
          <Image src={placeHolder} height={height || 50} width={width || 50} alt={name?.name} unoptimized />
        </div>
      ) : NameWithRound ? (
